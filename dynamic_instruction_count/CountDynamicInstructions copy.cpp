...
struct CountDynamicInstructions : public ModulePass {
    ...
    bool runOnModule(Module &M) override {
                ...
                for (auto I = B->begin(); I != B->end(); I++) {
                    ...
                    if (isa<ReturnInst>(I)) {
                        ...
                        if (const Function *call_func = dyn_cast<Function>(CI->getCalledValue()->stripPointerCasts()))
                        {
                            ...
                            is_exit |= (caller == "exit" || caller == "f90_stop08a" ||
        ...
        if(!ptr_instrumented) llvm::outs() << "[printOutInstrInfo is not instrumented]" << "\n";
        return false;
    }
        ...
        for (Function::iterator B = F.begin(); B != F.end(); ++B) {
            ...
            for (BasicBlock::iterator I = B->begin(); I != B->end(); ++I) {
                ...
                if(isa<CallInst>(I) || isa<InvokeInst>(I)) {
                    ...
                    bool is_exit = false;
                    if (const Function *call_func = CS.getCalledFunction()) {
                        ...
                        is_exit |= (caller == "exit" || caller == "f90_stop08a" ||
                                    caller.find("quit_flag_") != std::string::npos);
                        // debug
                        FunctionCallee prt_ = F.getParent()->getOrInsertFunction(
                            "printOutCalledFunction", Type::getVoidTy(F.getParent()->getContext()),
                            Type::getInt8PtrTy(F.getParent()->getContext()));
                        IRBuilder<> builder_(&*I);

                        std::vector<Value *> args = {
                            builder_.CreatePointerCast(
                                builder_.CreateGlobalStringPtr(caller), 
                                Type::getInt8PtrTy(F.getParent()->getContext())
                            )
                        };
                        builder_.CreateCall(prt_, args);
                        llvm::outs() << "[printOutCalledFunction instrumented] "
                                    << "Function: " << F.getName() << ", "
                                    << "Instruction: " << *I << "\n";
                    }
                    if (!is_exit) continue;
